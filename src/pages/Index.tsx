...
import InstallPwaButton from "@/components/InstallPwaButton"; // Importando o novo componente
import { Loader2, User } from "lucide-react";

// --- Data Fetching Utilities ---

// Fetching Public Profile
const fetchPublicProfile = async (): Promise<PublicProfile> => {
  // Busca o primeiro perfil disponível (assumindo Linktree de usuário único)
  const { data, error } = await supabase
    .from("profiles")
    .select("id, first_name, last_name, bio, avatar_url, store_hours, address, sales_pitch") // Adicionando novos campos
    .limit(1)
    .single();
  ...
  const defaultProfile: PublicProfile = {
    id: 'default', // Placeholder ID if no profile exists
    first_name: "ELF JOPLIN",
    last_name: null,
    bio: "Seu hub de links essenciais.",
    avatar_url: "/logo.png",
    store_hours: null,
    address: null,
    sales_pitch: null,
  ...
  return {
    id: data.id,
    first_name: data.first_name || defaultProfile.first_name,
...
const fetchSections = async (userId: string): Promise<(Section & { content_url: string | null })[]> => {
  ...
  const { data, error } = await supabase
    .from("sections")
    .select("*, content_url") // Buscando content_url
...
const fetchAllActiveLinksGrouped = async (sectionIds: string[]): Promise<SectionLinkItem[]> => {
  ...
  // Fetch all active links and their section assignments for the given sections
  const { data, error } = await supabase
    .from("section_links")
    .select(`
      section_id,
...
const fetchTestimonials = async (userId: string): Promise<Testimonial[]> => {
  ...
  // NOTE: Testimonials are currently global per user, not per section.
  // If we need per-section testimonials later, we'd need a section_testimonials table.
  const { data, error } = await supabase
    .from("testimonials")
    .select("*")
    .eq("user_id", userId) // Filtrar por user_id
...
const Index = () => {
  const { data: profile, isLoading: isLoadingProfile, isError: isErrorProfile } = useQuery<PublicProfile>({
    queryKey: ["publicProfile"],
    queryFn: fetchPublicProfile,
  });
  
  const userId = profile?.id;
  const isDefaultProfile = userId === 'default';
  ...
  const { data: sections, isLoading: isLoadingSections, isError: isErrorSections } = useQuery<(Section & { content_url: string | null })[]>({
    ...
    enabled: !!userId && !isDefaultProfile,
  });
  ...
  // NOTE: Testimonials are still fetched globally per user, as they are not yet managed per section.
  const { data: testimonials, isLoading: isLoadingTestimonials, isError: isErrorTestimonials } = useQuery<Testimonial[]>({
    ...
    enabled: !!userId && !isDefaultProfile && activeTestimonialSectionIds.length > 0,
  });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-gray-500">Tente recarregar a página ou verifique a configuração do Supabase.</p>
        </div>
      </div>
    );
  }
